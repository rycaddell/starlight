...
export async function shareMirror(mirrorId, senderUserId, recipientUserId) {
  try {
    ...
    if (existingShare) {
      ...
      return {
        success: false,
        error: 'You've already shared this mirror with them',
      };
    }
...
export async function fetchIncomingShares(userId) {
  try {
    ...
    const formattedShares = (shares || []).map((share) => ({
      ...
      mirror: {
        id: share.mirrors.id,
        screen1Themes: share.mirrors.screen_1_themes,
        screen2Biblical: share.mirrors.screen_2_biblical,
        screen3Observations: share.mirrors.screen_3_observations,
        createdAt: share.mirrors.created_at,
        journalCount: share.mirrors.journal_count,
      },
    }));
 ...
 * Future: Can add a 'viewed_at' column to track this properly
 * For MVP: Return total incoming share count
 * @param {string} userId - UUID of user
  ...
  try {
    ...
    return {
      ...
      mirror: {
        ...
        screen2Biblical: share.mirrors.screen_2_biblical,
        screen3Observations: share.mirrors.screen_3_observations,
        screen4Suggestions: share.mirrors.screen_4_suggestions,
        createdAt: share.mirrors.created_at,
        journalCount: share.mirrors.journal_count,
      },
      senderName: share.users?.display_name || 'Unknown',
