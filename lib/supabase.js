...
console.log('🔍 openaiApiKey:', openaiApiKey ? 'Found' : 'Missing');
...
/**
 * Legacy Authentication Helper Functions (Keep for backward compatibility)
 */
...
/**
 * ACCESS CODE AUTHENTICATION FUNCTIONS (FIXED WITH SESSION MANAGEMENT)
 */


// Replace your debug signInWithAccessCode function with this clean version:

export const signInWithAccessCode = async (accessCode) => {
  try {
    console.log(`🔑 Attempting sign-in with code: ${accessCode}`);
    
    // Look up the access code in our user_codes table
    const { data: userCode, error: lookupError } = await supabase
      .from('user_codes')
      .select('*')
    ...
    if (userCode.supabase_user_id) {
      // Existing user - try to restore their session
      console.log('📋 Found existing user, restoring session...');
      
      // Check if we have a stored session for this user
      const storedSession = await AsyncStorage.getItem(`starlight_session_${userCode.supabase_user_id}`);
      ...
      if (storedSession) {
        try {
          ...
          if (!error && data.user) {
            ...
            console.log('⚠️ Session restore failed, creating new session');
      ...
      // If session restore failed, create new anonymous user
      if (!supabaseUser) {
        ...
        // Update the user_codes record to point to new session
        await supabase
          .from('user_codes')
        ...
        if (session) {
          await AsyncStorage.setItem(
            `starlight_session_${supabaseUser.id}`, 
      ...
      // New user - create Supabase user and link to code
      ...
      // Link the Supabase user to our access code
      await supabase
        .from('user_codes')
        .update({ supabase_user_id: supabaseUser.id })
        .eq('id', userCode.id);
    ...
    // Store the access code locally for future auto-signin
    ...
    return { 
      success: true, 
      user: supabaseUser,
      displayName: userCode.display_name,
      isNewUser: isNewUser
    };
...
// Auto sign-in with stored access code
...
// Clear stored access code (for testing/logout)
export const clearStoredAccessCode = async () => {
  try {
    // Get current user ID to clear their session
    const { data: { user } } = await supabase.auth.getUser();
    
    // Clear stored session if we have a current user
    if (user) {
      await AsyncStorage.removeItem(`starlight_session_${user.id}`);
    }
    
    // Clear access code and sign out
...
// Create a new access code (for your testing/admin use)
export const createAccessCode = async (accessCode, displayName) => {
  try {
    const { data, error } = await supabase
      .from('user_codes')
      .insert([
...
export const checkAndGenerateMirror = async (userId) => {
  try {
    ...
    // Use ALL unassigned journals for this Mirror
    ...
    return { 
      ...
      journalsRemaining: 0 // Always resets to 0 after Mirror generation
    };
