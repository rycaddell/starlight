...
| 4 | Bug 1: Ensure AsyncStorage job always writes `journalId` even on error path | Silent lost recordings | `hooks/useAudioRecording.tsx` | ✅ Done |
| 5 | Bug 2: Distinguish network error from transcription pending in poll | Misleading "Still Transcribing" | `hooks/useAudioRecording.tsx` | ✅ Done |
| 6 | Fix `friends.js` unguarded DB queries in `acceptInvite()` | Silent friend invite crashes | `lib/supabase/friends.js` | ✅ Done |
